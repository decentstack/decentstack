...
const sos = require('../../decentpass-core/lib/sanityproxy')
...
class PeerConnection extends EventEmitter {
  constructor (initiator, exchangeKey, opts = {}) {
    ...
    // TODO: this.exchangeKey = hash(inputKey + 'hypertalk:v1')
    this.exchangeKey = exchangeKey

    // Pre-bind our listeners before registering them, to be able to safely remove them
    this.kill = this.kill.bind(this)

    this.stream = new Protocol(initiator, sos({
      onhandshake: this._onHandshake.bind(this),
      onremoteopen: this._onRemoteFeedJoined.bind(this)
    }, 'protohandler'))

    this.virtualFeed = this.stream.open(this.exchangeKey, sos({
    ...
    this.debug('Initializing new PeerConnection, extensions:', this._rextlut)
  ...
  extension (name, ...args) {
    ...
    this.virtualFeed.extension(id, ...args)
  }

  _onHandshake () {
    this.debug(`Handshake from`)
    // TODO: If live exists then degrade to live
    // otherwise exchange morecaps?
    return
    try {
  ...
  _transition (newState, err = null) {
    ...
    switch (this.state) {
      ...
      case STATE_ACTIVE:
        ...
        if (this.opts.live) this.stream.wait()
  ...
  kill (err = null) {
    ...
    const cleanup = () => {
      ...
      if (this.opts.live) this.stream.continue()
      this.virtualFeed.close()
  ...
  _onRemoteFeedJoined (discoveryKey) {
    console.log('_onRemoteFeedJoined', arguments)
    debugger
