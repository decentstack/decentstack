...
class PeerConnection extends EventEmitter {
  ...
  joinFeed (feed, cb) {
    ...
    // TODO: i give up, the `expectedFeeds` is badly implemented
    // in numerous applications and there is no way for a
    // replication manager to extract the info.
    // HyperdriveV10 uses corestore but corrupts the main stream
    // so we're going to make a workaround using virtual-streams.

    const virtualStream = substream(this.stream, feed.key)
    ...
    eos(virtualStream, err => {
      if (err) this.kill(err) // Kills the peer if a substream fails
      this.stream.expectedFeeds-- // simulate feed disconnect
      // this.stream.maybeFinalize() // invoke protocol logic
...
// This is a workaround to awkward design of expectedFeeds.
// we're going to highjack the first expectedFeeds set.
// THE RABBITHOLE GOES DEEPER THAN I EXPECTED
// const streamInitProxy = (stream, cb) => {
//  if (typeof cb !== 'function') throw new Error('Callback not provided')
//  let yanked = false
...
//          const [, opts] = a
//          debugger
//          return Reflect.apply(target[prop], undefined, a)
//        }
//      }
//      [>
//      if (prop === 'expectedFeeds') debugger //console.log('GET\t\t', prop)
//      if (!yanked && prop === 'expectedFeeds') return 0 // Pretend a zero
//      */
//      return v
//    },
//    set (target, prop, args) {
//      if (prop === 'expectedFeeds') console.log('SET\t\t', prop, args)
//      if (prop === 'expectedFeeds') {
//        debugger
//        yanked = true
//        cb(args) // yank
//        return true
//      } else return Reflect.set(target, prop, args)
//    }
//  })
// }
